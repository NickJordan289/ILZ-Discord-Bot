...
if debug:
    print('USDC-ETH conversion', usdc_conversion)
aud_conversion = helpers.get_aud_price()
if debug:
    print('ETH-AUD conversion', aud_conversion)

scanner_df = pd.read_csv('plot_scanners_all_final_sorted.csv')
...
async def send_message(row, tokentrove_data):
    ...
    max_scanners = scanner_df[scanner_df['token_id']
                              == int(asset_id)]['max_scanners'].values[0] if len(scanner_df[scanner_df['token_id'] == int(asset_id)]) > 0 else 0
    ...
    async with aiohttp.ClientSession() as session:
        ...
        embed = Embed(title=title, url=url, description=plot_name,
        ...
        embed.add_field(name="Balanced Elements",
                        value=balanced_element, inline=False)
        if tier > 1:
            embed.add_field(name="Balanced Fuel",
                            value=balanced_fuel, inline=False)
        embed.add_field(name="Max Scanners",
                        value=max_scanners, inline=False)
        embed.add_field(
            name=f"Less than Floor ({tier_floor_price:.4f} ETH)", value=less_than_floor, inline=True)
        embed.add_field(name=f"Listing User",
                        value=f'[{user_name}](https://illuvidex.illuvium.io/ranger/{user_name})', inline=False)
        try:
            import_url = helpers.get_import_url(asset_id)
            embed.add_field(name="Simulator Link",
                            value=f"[View Plot]({import_url})", inline=False)
        except Exception as e:
...
if __name__ == '__main__':
    ...
    while True:
        try:
            if debug:
                print(f'Looking for new listings since {last_timestamp}')
            df = helpers.load_imx(
                last_timestamp, usdc_conversion, aud_conversion)
            if len(df) > 0:
                ...
                if debug:
                    print('Updated timestamp to', last_timestamp)
            else:
                if debug:
