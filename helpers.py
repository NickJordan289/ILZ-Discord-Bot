...
def get_usdc_price():
    response = requests.get('https://api.coingecko.com/api/v3/simple/price?ids=usd-coin&vs_currencies=eth').json()
    usdc_conversion = float(response['usd-coin']['eth'])
    return usdc_conversion
def get_aud_price():
    response = requests.get('https://api.coingecko.com/api/v3/simple/price?ids=ethereum&vs_currencies=aud').json()
...
def load_imx_erc20(timestamp, usdc_conversion, aud_conversion):
    ...
    df['erc20'] = df['buy_data'].apply(lambda x: int(x['quantity_with_fees'])/1e6)
    df['eth'] = df['erc20'].apply(lambda x: usdc_to_eth(x, usdc_conversion) if x > 0 else 0)
    df['aud'] = df['eth'].apply(lambda x: eth_to_aud(x, aud_conversion) if x > 0 else 0)
    return df
...
def load_imx_eth(timestamp, aud_conversion):
    ...
    df['buy_data'] = df['buy'].apply(lambda x: x['data'])
    df['decimals'] = df['buy_data'].apply(lambda x: x['decimals'])
    df['eth'] = df['buy_data'].apply(lambda x: int(x['quantity_with_fees'])/1e18)
    df['erc20'] = 0
    df['aud'] = df['eth'].apply(lambda x: eth_to_aud(x, aud_conversion) if x > 0 else 0)
    return df
...
def load_imx(timestamp, usdc_conversion, aud_conversion):
    ...
    df['url'] = df['asset_id'].apply(lambda x: f'https://illuvidex.illuvium.io/asset/0x9e0d99b864e1ac12565125c5a82b59adea5a09cd/{x}')
    ...
    df['coordinates'] = df['name'].apply(lambda x: x.split('(')[1].split(')')[0])
